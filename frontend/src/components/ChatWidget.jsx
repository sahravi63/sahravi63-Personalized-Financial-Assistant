...
export default function ChatWidget() {
  ...
  const bubbleStyle = useMemo(() => ({
    position: 'fixed', right: 18, bottom: 18, width: 340, borderRadius: 18, background: 'linear-gradient(145deg, #111827, #1f2937)', border: '1px solid #334155', color: '#fff', boxShadow: '0 18px 45px rgba(15, 23, 42, 0.45)', zIndex: 1000,
  }), []);

  return (
    <div style={bubbleStyle}>
      <button type="button" onClick={() => setOpen((prev) => !prev)} style={{ width: '100%', border: 'none', background: 'linear-gradient(135deg, #60a5fa, #8b5cf6)', color: '#fff', padding: 12, borderRadius: 18, cursor: 'pointer', fontWeight: 700 }}>
      ...
      {open && (
        <div style={{ padding: 12 }}>
          ...
          <form onSubmit={sendMessage} style={{ display: 'grid', gap: 8 }}>
            <input value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Ask a finance question" style={{ borderRadius: 10, border: '1px solid #334155', background: '#0f172a', color: '#fff', padding: '10px 12px' }} />
            <button type="submit" disabled={loading} style={{ borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #34d399, #22d3ee)', color: '#0f172a', padding: '10px 12px', cursor: 'pointer', fontWeight: 700 }}>Send</button>
          </form>
        </div>
      )}
    </div>
